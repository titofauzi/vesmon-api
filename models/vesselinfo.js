...
return db.query("Select * from vessel_info limit 10",callback);
 ...
 getAllVesselinfoByTime:function(date,callback){
 
 var sql = "select bb.* from (select a.mmsi,a.tipe,c.lon,c.lat,c.hdt,c.sog,c.last_posdate, a.name, a.callsign from vessel_info a inner join ( select * from ( select * from ( select mmsi,last_posdate, lat,lon,sog,hdt,cog, CASE mmsi WHEN @curType THEN @curRank := @curRank + 1 ELSE @curRank := 1 AND @curType := mmsi END AS rank from vessel_track a, (SELECT @curRank := 0, @curType := '') r where last_posdate > DATE_SUB(STR_TO_DATE(?,'%Y-%m-%d %H:%i:%s'),INTERVAL 9 HOUR) and last_posdate < DATE_SUB(STR_TO_DATE(?,'%Y-%m-%d %H:%i:%s'), INTERVAL 7 HOUR) ORDER BY MMSI, last_posdate desc ) x where x.rank = 1 ) dd ) c on c.mmsi=a.mmsi and a.callsign in (SELECT callsign FROM pertamina_ship)) bb LEFT JOIN vessel_type b ON (bb.tipe = b.tipe_name)order by bb.mmsi asc,bb.last_posdate asc";

return db.query(sql,[date, date],callback);
 },
